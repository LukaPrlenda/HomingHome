...
class InterestDao extends BaseDao{
    ...
    public function get_by_status_and_interested_id($status, $user_id){
        return $this->query('SELECT p.*, i.id AS interest_id, i.user_id AS intrested_user_id, i.status, i.message, u.name AS intrested_name, u.surname AS intrested_surname, u.phone_number AS intrested_phone, u.email AS intrested_gmail FROM ' . $this->table_name . ' i JOIN properties p ON i.property_id = p.id JOIN users u ON i.user_id = u.id WHERE i.user_id = :user_id AND i.status = :status', ['status' => $status, 'user_id' => $user_id]);
    }
    ...
    public function get_by_status_and_owner_id_first_N($status, $user_id, $number){
        return $this->query('SELECT p.*, i.id AS interest_id, i.user_id AS intrested_user_id, i.status, i.message, u.name AS intrested_name, u.surname AS intrested_surname, u.phone_number AS intrested_phone, u.email AS intrested_gmail FROM ' . $this->table_name . ' i JOIN properties p ON i.property_id = p.id JOIN users u ON i.user_id = u.id WHERE p.user_id = :user_id AND i.status = :status LIMIT ' . $number, ['status' => $status, 'user_id' => $user_id]);
    }

    public function get_by_status_interested_id_and_property_id($status, $user_id, $property_id){
        return $this->query('SELECT p.location FROM ' . $this->table_name . ' i JOIN properties p ON i.property_id = p.id WHERE i.user_id = :user_id AND i.status = :status AND i.property_id = :property_id', ['status' => $status, 'user_id' => $user_id, 'property_id' => $property_id]);
    }
