...
class ListingDao extends BaseDao{
    ...
    public function get_by_status_desc($status){
        return $this->query('SELECT p.*, l.status, l.id AS listing_id, t.type FROM ' . $this->table_name . ' l JOIN properties p ON l.property_id = p.id JOIN types t ON t.id = p.type_id WHERE l.status = :status ORDER BY p.location ASC', ['status' => $status]);
    }
